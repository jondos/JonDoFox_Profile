...
function downbarInit() {
	...
	// This needs a delayed startup because for some reason it breaks the back button in Linux and Mac when run in line, see bug 17384
	window.setTimeout(function(){
		db_setupTooltips();
	}, 10);
	...
	// For unknown reasons, sometimes session store does not get turned back on after a 
	// pre-browser window "donate" session in addonDonate.js
	// This is a double-check safety measure to make sure it gets turned back on
	try {
		if(db_pref.getBoolPref("downbar.function.ensureSessionState")) {
			db_pref.setBoolPref("browser.sessionstore.enabled", true);
			db_pref.clearUserPref("downbar.function.ensureSessionState");
		}
	} catch(e) {}
...
function db_setStyles() {
	...
	if(styleDefault) {
		...
		//Set the background of the buttons to the same color as the statusbar background (using the downbar mini popup style color which is the same)
		document.getElementById("downbarHolder").setAttribute("style", downbarPopupStyle);
	}
	//downbarelem.builder.rebuild();  db_checkShouldShow(); db_startInProgress();
...
*/
		...
		elem.setAttribute("pTimeCode", window.setTimeout(function(){db_makeTooltip(dlElem);}, 1000));
	} catch(e) {
		document.getElementById("progresstip").hidePopup();
	}
}
