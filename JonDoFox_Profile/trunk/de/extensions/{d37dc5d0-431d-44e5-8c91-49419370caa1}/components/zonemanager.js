...
function FoxClocks_Zone(zoneNode)
{
	...
	this.st_to_string = standardNode.getAttribute("offset");
	...
	if (daylightNodes.length == 1)
	{
		...
		this.dl_to_string = daylightNode.getAttribute("offset");
...
function FoxClocks_ZoneManager()
{
	...
	this.zones = null;
	this.zonePickerLocationMap = null;
...
{	
	...
	// ====================================================================================
	initZoneData: function(zoneDataURL)
	{
		...
		// AFM - generate zone data
		//
		this.logger.log("FoxClocks_ZoneManager::initZoneData(): zoneDataURL is " + zoneDataURL);		

		httpRequest.open('GET', zoneDataURL, false);
		...
		{
			this.logger.log("FoxClocks_ZoneManager::initZoneData(): parsererror: " +
					root.firstChild.nodeValue, this.logger.ERROR);
			return false;
		}
	...
	// ====================================================================================
	initZonePicker: function(zonePickerDataUrl)
	{
		...
		{
  			var httpRequest = CC["@mozilla.org/xmlextras/xmlhttprequest;1"].createInstance(CI.nsIXMLHttpRequest);
			httpRequest.open('GET', zonePickerDataUrl, false);
			httpRequest.send(null);
			this.zonePickerXmlDoc = httpRequest.responseXML;
			
			// AFM - this could happen for a remote, rather than chrome, URL
			//
			if (this.zonePickerXmlDoc == null)
				throw "Bad response";
	
			zpRoot = this.zonePickerXmlDoc.documentElement;
			
			if (zpRoot.nodeName == "parsererror")
				throw zpRoot.firstChild.nodeValue;
		}
		catch(ex) 
		{
			this.logger.log("FoxClocks_ZoneManager::initZonePicker(): cannot retrieve valid data from <" +
				zonePickerDataUrl + ">: " + ex, this.logger.ERROR);
			return false;
		}
	
		var utilsService = CC["@stemhaus.com/firefox/foxclocks/utils;1"].getService(CI.nsISupports).wrappedJSObject;	
		if (zpRoot.namespaceURI != utilsService.FC_URI_ZONEPICKER_NAMESPACE || zpRoot.localName != "zonepicker")
		{
			this.logger.log("FoxClocks_ZoneManager::initZonePicker(): url <" +
				zonePickerDataUrl + "> is not a valid FoxClocks ZonePicker file", this.logger.ERROR);

			return false;
		}
					
		var zpLeafNodes = zpRoot.getElementsByTagName("Leaf");
