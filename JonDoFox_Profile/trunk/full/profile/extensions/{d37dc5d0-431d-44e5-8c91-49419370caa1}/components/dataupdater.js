...
FoxClocks_UpdateManager.prototype =
{
	...
	{		
		if (this._httpRequest.readyState == this._utils.XHR_STATE_NOT_INIT)
			return;
		
		this._logger.log("+FoxClocks_UpdateManager::_resetConnection(): state <" +
				this._utils.XHR_STATE_STRINGS[this._httpRequest.readyState] + ">");
					
		if (this._httpRequest.readyState != this._utils.XHR_STATE_LOADED)
		{
			...
			this._logger.log("FoxClocks_UpdateManager::_resetConnection(): aborting current request with state: " +
				this._utils.XHR_STATE_STRINGS[this._httpRequest.readyState], this._logger.WARN);
		}
	...
	{		
		...
		if (state != self._utils.XHR_STATE_RECEIVING)
		{
			self._logger.log("+FoxClocks_UpdateManager::_onXMLReadyStateChange(): state <" +
				self._utils.XHR_STATE_STRINGS[state] + ">");
		}
		...
		if (httpStatus != null)
		{
			...
			//
			if (doc != null && (httpStatus == self._utils.HTTP_STATUS_OK || (self._noServerUpdateURL != null && httpStatus == 0)))
			{
	...
	_processXMLResponse: function(doc)
	{
		...
		try
		{
			...
			{						
				...
				//		
				var outConvStream = CC["@mozilla.org/intl/converter-output-stream;1"]
