...
angular.module('VDH', ['ngAnimate']).run(['VDH.util',function() {
}]);
	...
	[ '$window','$rootScope',
	  function($window,$rootScope) {
		  this.prepareScope = function($scope) {
				$scope.strings = self.options.l10nData;
				$scope.post = function(type,data) {
					self.postMessage(angular.extend({
						type: type,
					},data || {}));
				}
				...
				if(self.options.initData)
					angular.extend($scope,self.options.initData);
		  }
}]);
