...
VIAddVersionKey LegalCopyright "JonDos GmbH, John T. Haller, portions Gerard Balagué"
VIAddVersionKey FileDescription "${PORTABLEAPPNAME}"
VIAddVersionKey FileVersion "${VER}"
VIAddVersionKey ProductVersion "${VER}"
VIAddVersionKey InternalName "${PORTABLEAPPNAME}"
VIAddVersionKey LegalTrademarks "Firefox is a Trademark of The Mozilla Foundation.  PortableApps.com is a Trademark of Rare Ideas, LLC. JonDoFox is a trademark of the JonDos GmbH."
...
Section "Main"
	...
	WarnAnotherInstance:
		...
		MessageBox MB_YESNO|MB_ICONQUESTION `Firefox is already running. Close Firefox and continue starting ${APPNAME}?` IDNO "" IDYES KeepLoadingFirefox
		;MessageBox MB_YESNO|MB_ICONQUESTION `Firefox läuft bereits. Soll Firefox beendet werden und ${APPNAME} gestartet werden?` IDNO "" IDYES KeepLoadingFirefox
		Abort	
	...
	KeepLoadingFirefox:
		...
		Pop $R0 
		Sleep 1000		
		StrCmp $R0 "603" AfterKill
    StrCmp $R0 "0"  AfterKill
    MessageBox MB_OK "Could not close Firefox! Error code: $R0"
    Abort
		;Goto ProfileWork	
	...
	CheckForLicense:
		...
		StrCmp $0 "${LICENSEVERSION}" LicenseAgreedTo RelaunchWithLicense
